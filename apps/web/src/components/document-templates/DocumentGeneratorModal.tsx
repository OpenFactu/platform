...
export const DocumentGeneratorModal: React.FC<Props> = ({ templateId, templateName, onClose }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/document-templates/${templateId}`, { headers });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const full = (await res.json()) as { canvasLayout?: CanvasLayout | null };
        if (cancelled) return;
          ...
          defs
            ...
            .map(async (p) => {
              if (p.optionsQuery) {
                try {
                  const r = await fetch(
                    `/api/document-templates/${templateId}/param-options`,
                    {
                      method: 'POST',
                      headers: { ...headers, 'Content-Type': 'application/json' },
                      body: JSON.stringify({ param: p.name }),
                    },
                  );
                  const b = await r.json();
                  optMap[p.name] = Array.isArray(b.options) ? b.options : [];
                } catch {
                  optMap[p.name] = [];
                }
              } else {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/document-templates/${templateId}/render-free`, {
        method: 'POST',
        headers: { ...headers, 'Content-Type': 'application/json' },
      ...
      const errHeader = res.headers.get('X-Render-Free-Errors');
