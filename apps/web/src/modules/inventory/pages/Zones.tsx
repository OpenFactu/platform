...
export const Zones: React.FC = () => {
  ...
  const fetchZones = async () => {
    setLoading(true);
    try {
      const res = await coreApi.raw('GET', '/api/zones');
      const data = res.data;
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await coreApi.raw('POST', '/api/zones', newRow);
      if (res.ok) {
        setNewRow(null);
        fetchZones();
  ...
  const handleUpdate = async (id: string, data: any) => {
    try {
      const res = await coreApi.raw('PATCH', `/api/zones/${id}`, data);
      if (res.ok) {
        setEditingId(null);
        fetchZones();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await coreApi.raw('DELETE', `/api/zones/${id}`);
      if (res.ok) {
        fetchZones();
