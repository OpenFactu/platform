    ...
    <div className="p-4 space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border-subtle pb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-fg-default tracking-tighter flex items-center gap-3">
...
export const SalesInvoices: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await docsApi.list('/api/sales/invoices');
  ...
  const handlePost = async () => {
    ...
    try {
      setPosting(true);
      await docsApi.post('/api/sales/invoices', selectedInvoice.id);
