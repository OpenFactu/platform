...
const SDNList: React.FC<{
  ...
  return (
    <div className="p-4 space-y-8 animate-in fade-in duration-500">
      <PageHeader
        title="Salidas (Albaranes)"
        subtitle="Registro físico de salida de productos y control de expedición."
        icon={<Truck size={18} />}
        size="lg"
        divider
        toolbar={
...
export const SalesDeliveryNotes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await docsApi.list('/api/zones');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await docsApi.list('/api/sales/delivery-notes');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await docsApi.get('/api/sales/delivery-notes', detailId);
  ...
  const handleCancel = async (id: string) => {
    ...
    try {
      ...
      toast.error((e instanceof Error ? e.message : undefined) || 'Error al cancelar');
    }
  };
